...
class Weixin_lib{
    ...
    public static function getInstance($options=array()) {
        static $objPools = array();
        if(false == $options) {
            if(defined('WEIXIN_TOKEN'))$options['token'] = WEIXIN_TOKEN;
            if(defined('WEIXIN_APPID'))$options['appid'] = WEIXIN_APPID;
            if(defined('WEIXIN_SECRET'))$options['appsecret'] = WEIXIN_SECRET;
        }
        $optKey = md5(serialize($options));
        if(false == $objPools[$optKey]) {
            $objPools[$optKey] = new WeixinAPI($options);
        }
        return $objPools[$optKey];
    }
    ...
    public function getRev()
    {
        ...
        if(WX_DEBUG && false == $postStr) {
            $postStr = file_get_contents(DBG_TMP_DIR . '/post');
        }
        if($postStr)
        {
            if(WX_DEBUG ) {
                file_put_contents(DBG_TMP_DIR . '/post', $postStr);
            }
    ...
    public function createMenu($data)
    {
        ...
        $result = self::curlRequest(self::API_URL_PREFIX.self::MENU_CREATE_URL.'access_token='.$this->access_token, $this->jsonEncode($data), 'post');
        if($result)
        {
    ...
    public function getMenu()
    {
        ...
        $result = self::curlRequest(self::API_URL_PREFIX.self::MENU_GET_URL.'access_token='.$this->access_token);
        if($result)
        {
            $jsonArr = json_decode($result, true);
            if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
    ...
    public function deleteMenu()
    {
        ...
        $result = self::curlRequest(self::API_URL_PREFIX.self::MENU_DELETE_URL.'access_token='.$this->access_token);
        if($result)
        {
            $jsonArr = json_decode($result, true);
            if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
    ...
    public function getUserInfo($openid)
    {
        ...
        $result = self::curlRequest(self::API_URL_PREFIX.self::USER_INFO_URL.'access_token='.$this->access_token.'&openid='.$openid);
        if($result)
        {
            $jsonArr = json_decode($result, true);
            if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
    ...
    public function getUserList($next_openid='')
    {
        ...
        $result = self::curlRequest(self::API_URL_PREFIX.self::USER_GET_URL.'access_token='.$this->access_token.'&next_openid='.$next_openid);
        if($result)
        {
            $jsonArr = json_decode($result, true);
            if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
    ...
    public function getGroup()
    {
        ...
        $result = self::curlRequest(self::API_URL_PREFIX.self::GROUPS_GET_URL.'access_token='.$this->access_token);
        if($result)
        {
            $jsonArr = json_decode($result, true);
            if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
    ...
    public function createGroup($name)
    {
        ...
        $result = self::curlRequest(self::API_URL_PREFIX.self::GROUPS_CREATE_URL.'access_token='.$this->access_token, $this->jsonEncode($data), 'post');
        if($result)
        {
            $jsonArr = json_decode($result, true);
            if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
    ...
    public function updateGroup($id, $name)
    {
        ...
        $result = self::curlRequest(self::API_URL_PREFIX.self::GROUPS_UPDATE_URL.'access_token='.$this->access_token, $this->jsonEncode($data), 'post');
        if($result)
        {
            $jsonArr = json_decode($result, true);
            if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
    ...
    public function updateGroupMembers($openid, $to_groupid)
    {
        ...
        $result = self::curlRequest(self::API_URL_PREFIX.self::GROUPS_MEMBERS_UPDATE_URL.'access_token='.$this->access_token, $this->jsonEncode($data), 'post');
        if($result)
        {
            $jsonArr = json_decode($result, true);
            if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
    ...
    public function sendCustomMessage($touser, $data, $msgType = 'text')
    {
        ...
        $result = self::curlRequest(self::API_URL_PREFIX.self::MESSAGE_CUSTOM_SEND_URL.'access_token='.$this->access_token, $this->jsonEncode($arr), 'post');
        if($result)
        {
            $jsonArr = json_decode($result, true);
            if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
    ...
    public function sendToUser($openId, $data) {
    ...
    {
        if(false == defined('WX_TOKEN_FILE')) {
            define('WX_TOKEN_FILE', '/dev/shm/wxToken/' . WEIXIN_APPID);
        }
        // 从缓存中获取access_token
        @mkdir(dirname(WX_TOKEN_FILE));
        if(file_exists(WX_TOKEN_FILE)) {
            $tokenInfo = unserialize(file_get_contents(WX_TOKEN_FILE));
        ...
        $result = self::curlRequest(self::API_URL_PREFIX.self::AUTH_URL.'appid='.$this->appid.'&secret='.$this->appsecret);
        if($result)
        {
            $jsonArr = json_decode($result, true);
            ...
            else
            {
                ...
                // 将access_token保存到缓存中
                #$cache->set('Weixin', $cache_flag, $this->access_token, $expire); 
                $arr = array('expire'=>$expire, 'accessToken'=>$this->access_token);
                file_put_contents(WX_TOKEN_FILE, serialize($arr));
                return true;
            }
        }
        return false;
    ...
    {
        $result = self::curlRequest(self::SNS_OAUTH_ACCESS_TOKEN_URL.'appid='.$this->appid.'&secret='.$this->appsecret.'&code='.$code.'&grant_type=authorization_code');
        if($result)
        {
            $jsonArr = json_decode($result, true);
            if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
    ...
    {
        $result = self::curlRequest(self::SNS_OAUTH_REFRESH_TOKEN_URL.'appid='.$this->appid.'&grant_type=refresh_token&refresh_token='.$refresh_token);
        if($result)
        {
            $jsonArr = json_decode($result, true);
            if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
    ...
    {
        $result = self::curlRequest(self::SNS_USERINFO_URL.'access_token='.$access_token.'&openid='.$openid);
        if($result)
        {
            $jsonArr = json_decode($result, true);
            if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
    ...
    public function createQrcode($scene_id, $type=0, $expire=1800)
    {
        ...
        $result = self::curlRequest(self::API_URL_PREFIX.self::QRCODE_CREATE_URL.'access_token='.$this->access_token, $this->jsonEncode($data), 'post');
        if($result)
        {
            $jsonArr = json_decode($result, true);
            if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0)) $this->error($jsonArr);
    ...
    public function error($data)
    {
        ...
        return 'WEIXIN API errcode:['.$this->errCode.'] errmsg:['.$this->errMsg.']';
    }
